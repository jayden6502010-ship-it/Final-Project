// Upgrade NOTE: replaced 'defined _AKMU_MIRROR_RIPPLE' with 'defined (_AKMU_MIRROR_RIPPLE)'
...
float GetDepthTexture(float eyeIndex, float2 uv) {
    if (eyeIndex == 0.) {
        return tex2D(_ReflectionTexDepth, uv);
    }
    else {
        return tex2D(_ReflectionTexOtherDepth, uv);
    }
    /*if (eyeIndex == 0.) {
        return SAMPLE_DEPTH_TEXTURE(_ReflectionTexDepth, uv);
    }
    else {
        return SAMPLE_DEPTH_TEXTURE(_ReflectionTexOtherDepth, uv);
    }*/
}
//
...
float2 Index_float(in float isMultiPass, float2 screenUV, out float Eye) {
...
inline float4 CalculateReflection(float3 eyeVec, float4 screenPos, float texdistance, float eyeIndex,
    ...
    bool isForward, inout half3 emissiveColor, inout half texAlpha)
{
    ...
    //maskAlpha = SampleAlbedoAlpha(texuv / half2(_MaskTiling.r, _MaskTiling.g), TEXTURE2D_ARGS(_MaskTex, sampler_MaskTex));
    maskAlpha = tex2D(_MaskTex, texuv / half2(_MaskTiling.r, _MaskTiling.g)).a;

    mask = smoothstep(maskAlpha.a, 0, _MaskCutoff);

    //emissiveColor = mask.a; //test
    //return 1; //test
#endif
    ...
    Index_float(_IsMultiPass, screenUV, eyeIndex);

    //!!important eyeIndex is calculated inside related shader code, 
    //unity_stereoEyeIndex gets wrong if we try to get here!!!!
    //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    if (_Platform != 10.) //if platform is not VR, set the always 0i şt come wrong on android, AR vb!!!!!!!!!
    {
        eyeIndex = 0;
    }
    ...
    if (_ClipUV < 99) {
        ...
        //screenUV = input.uv;
        //todo: is it reversed?
        screenUV = 1 - texuv;// +_ClipPercentage / 1000.;// *(_ClipPercentage / 100.) + 0.5;
    }
    ...
    //texuv.xy += (nor * (_ReflectionRefraction / texdistance));
    ...
    float3 noise1 = UnpackNormal(tex2D(_WaveNoiseTex, noiseUV1 / _WaveSize));
    float3 noise2 = UnpackNormal(tex2D(_WaveNoiseTex, noiseUV2 / _WaveSize));

    noiseOffset = (noise1.xy + noise2.xy) / 2;
    noiseOffset -= float2(.5, .5);
    noiseOffset *= (_WaveDistortion / texdistance);
    ...
    //float4 temp_output_63_0 = (tex2D(_Mask, customUVs39, float2(0, 0), float2(0, 0)) * i.vertexColor);
        ...
        half3 ripNor = UnpackScaleNormal(tex2D(_RippleTex, fbuv58), _RippleDensity);
    ...
    input2 = (normal * (_RippleRefraction / texdistance)); //use as static //so far away pixels will not be refracted very much
    input1 = texuv - input2;//2_MainTex_ST; //will be used to animate real texture
    screenUV.xy -= input2;
    ...
    half3 bump = UnpackNormal(tex2D(_RefractionTex, texuv / _ReflectionRefraction)).rgb;
    ...
    if (_ReflectionIntensity > 0) {
        ...
        input1 = texuv + noiseOffset * mask.a;//will be used to animate real texture

        diffColor = tex2D(_MainTex, input1);
    }
#elif defined (_AKMU_MIRROR_RIPPLE)
    ...
    if (_UseFresnel > 0.5)
    {
        half NoV = saturate(dot(normal, -eyeVec));
...
#ifdef _AKMU_MIRROR_MASK
        if (isForward) {
            diffColor = diffColor * (1 - mask.a) +diffColor * mask.a * (1 - _ReflectionIntensity);
            emissiveColor = reflection.rgb * _ReflectionIntensity * pow(mask.a, _MaskEdgeDarkness);
        }
        else {
            diffColor = diffColor * (1 - mask.a) + diffColor * mask.a * (1 - _ReflectionIntensity);
            emissiveColor = emissiveColor * (1 - mask.a) + emissiveColor * mask.a * (1 - _ReflectionIntensity)
                + reflection.rgb * _ReflectionIntensity * pow(mask.a, _MaskEdgeDarkness);
        }
#else
        if (isForward) {
            ...
            diffColor = diffColor * (1 - _ReflectionIntensity);
            emissiveColor = emissiveColor * (1 - _ReflectionIntensity) + reflection.rgb * _ReflectionIntensity;
        }
#endif
    }
    else //transparent! 
    {
        ...
        {
            //emissiveColor = 0; //test
            //return 1; //test

            _ReflectionIntensity *= reflection.a;

            texAlpha = _ReflectionIntensity * mask.a;

            diffColor = 0;

            emissiveColor = reflection * texAlpha;
        }
        else {
            if (isForward) {
                diffColor = diffColor * texAlpha; //diffColor*(1-_ReflectionIntensity);
#ifdef _FULLMIRROR
                emissiveColor = reflection.rgb * _ReflectionIntensity * mask.a;
#else
                emissiveColor = reflection.rgb * _ReflectionIntensity * (1 - texAlpha) * mask.a;
#endif
                ...
                diffColor = diffColor * texAlpha; //diffColor*(1-_ReflectionIntensity);
#ifdef _FULLMIRROR
                emissiveColor = emissiveColor * (1 - _ReflectionIntensity) * mask.a
                    + reflection.rgb * _ReflectionIntensity * mask.a;
#else
                emissiveColor = emissiveColor * (1 - _ReflectionIntensity) * (1 - texAlpha) * mask.a
                    + reflection.rgb * _ReflectionIntensity * (1 - texAlpha) * mask.a;
#endif
            }
...
half4 fragForwardBaseInternal(VertexOutputForwardBase i)
{
    ...
    half3 emissiveColor;
    float4 reflection = CalculateReflection(s.eyeVec, i.screenPos, i.distance, i.eyeIndex, /*i.eyePos,*/ s.smoothness, s.specColor,
...
half4 fragForwardAddInternal(VertexOutputForwardAdd i)
{
    ...
    half3 emissiveColor;
    CalculateReflection(s.eyeVec, i.screenPos, i.distance, i.eyeIndex, /*i.eyePos,*/ s.smoothness, s.specColor,
        i.tex.xy, s.normalWorld, s.diffColor, true, emissiveColor, s.alpha);
